...
class SROSDriver(object):
    ...
    def get_interfaces(self):
        ...
        for interface in int_list:
            ...
            ipv4_status = re.search(' +\w+ +(\w+)/\w+', interface).group(1)
            ipv6_status = re.search(' +\w+ +\w+/(\w+)', interface).group(1)
            mode = re.search(' +\w+/\w+ +(\w+) ', interface).group(1)
            link_to = re.search(' +\w+/\w+ +\w+ +(\S+)\r\n', interface).group(1)
            try:
